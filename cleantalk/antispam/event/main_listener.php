	...
	*/
	...
	public function check_users_spam($event)
	{
					...
					if(isset($result->data))
					{
						foreach($result->data as $key=>$value)
						{
							if($key === filter_var($key, FILTER_VALIDATE_IP))
							{
								if($value->appears==1)
								{
									$sql = 'UPDATE ' . USERS_TABLE . ' set ct_marked=1 where user_ip="'.$key.'"';
									$result = $db->sql_query($sql);
								}
							}
							else
							{
								if($value->appears==1)
								{
									$sql = 'UPDATE ' . USERS_TABLE . ' set ct_marked=1 where user_email="'.$key.'"';
									$result = $db->sql_query($sql);
								}
							}
						}
		...
		if(strpos(__FILE__, 'cleantalk')!==false)
		{
			...
			$html.='<form method="post"><center><br /><table class="table1 zebra-table">
	<thead>
	<tr>
		<th>Select</th>
		<th>Username</th>
